...
def schedule_restoration_sequence_considering_GSCC_by_GA(config_json_path: str) -> str:
    """
    Single-input version.
    config_json_path: JSON file containing all parameters:
    {
        "global_json_path": "...",
        "population_size": 50,
        "generations": 100,
    ...
    global_json_path = config["global_json_path"]
    population_size = config.get("population_size", 50)
    generations = config.get("generations", 100)
    crossover_rate = config.get("crossover_rate", 0.8)
    mutation_rate = config.get("mutation_rate", 0.02)
    failure_file_path = config.get("failure_file_path", "cascading_failure_identification_under_big_nodes_attacks.json")
    ...
    for generation in range(generations):
        fitness_scores = [fitness(ind, network_data, edges) for ind in population]
        ...
        offspring = [mutation(ind, mutation_rate) for ind in offspring]
    ...
    for idx, node in enumerate(best_individual):
        ...
        if len(G) > 0:
            ...
            gscc_size = 0
        recovery_order.append({"step": idx+1, "node": node, "current_GSCC_size": gscc_size})

    output_json_path = 'recovery_sequence_considering_GSCC_by_GA.json'
    with open(output_json_path, 'w') as f:
        json.dump(recovery_order, f, indent=4)

    global_data["recovery_sequence_considering_GSCC_by_GA"] = output_json_path
    with open(global_json_path, 'w') as f:
        json.dump(global_data, f, indent=4)

    return f"The path to recovery order result has been saved in {global_json_path}"