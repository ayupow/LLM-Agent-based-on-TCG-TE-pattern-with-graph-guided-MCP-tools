...
def deploy_portable_equipment_considering_efficiency(global_json_path: str):
    # 加载全局数据文件
    ...
    # 保存结果到 JSON 文件
    output_json_path = 'failed_nodes_replacement_evaluated_by_efficiency.json'
    ...
    with open(global_json_path, 'r') as file:
        file_paths = json.load(file)
    file_paths['failed_nodes_replacement_evaluated_by_efficiency'] = output_json_path
    with open(global_json_path, 'w') as file:
        json.dump(file_paths, file, indent=4)

    print(f"failed nodes replacement evaluated by efficiency results saved to {output_json_path}")
