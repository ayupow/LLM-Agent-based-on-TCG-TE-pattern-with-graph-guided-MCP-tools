...
def deploy_portable_equipment_considering_population(global_json_path: str):
    ...
    output_json_path = 'failed_nodes_replacement_evaluated_by_population.json'
    ...
    with open(global_json_path, 'r') as file:
        file_paths = json.load(file)
    file_paths['failed_nodes_replacement_evaluated_by_population'] = output_json_path
    with open(global_json_path, 'w') as file:
        json.dump(file_paths, file, indent=4)

    print(f"failed nodes replacement evaluated by population results saved to {output_json_path}")
