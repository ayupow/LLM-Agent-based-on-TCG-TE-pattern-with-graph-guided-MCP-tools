...
def auto_update_knowledge_graph(kg_file='KGoLNR.json', schema_file='KGoLNR_schema_level.txt',
    ...
    if readme_files is None:
        readme_files = [
            'README_gis_servers.md', 'README_networkx_server.md',
    ...
    for rf in readme_files:
        try:
            ...
            print(f"  [WARN] README not found: {rf}")
    readme_summary = ""
...
```
IMPORTANT: Only output the JSON array, no other text. No duplicate tool names. Max 50 entries.
"""
    )
    print("\n" + "=" * 60)
    print("STEP 0: Auto-Updating Knowledge Graph from Server READMEs ...")
    print(f"  Existing tools: {len(existing_tools)}, READMEs: {len(readme_contents)}")
    chain = LLMChain(llm=model, prompt=kg_update_prompt)
    llm_response = chain.run(
        schema=schema_content, readme_summary=readme_summary,
    ...
    for entry in new_entries:
        try:
            sn = entry["p"]["start"]["properties"]["name"]
            en = entry["p"]["end"]["properties"]["name"]
...
def load_and_generate_tool_graph(input_json_file='KGoLNR_updated.json'):
    ...
    def build_tool_chains(rel_list):
        chain_list = []
        for s, e in rel_list:
            if s == "convert_SHPfile_to_JSONfile":
                chain_list.append([s, e])
        for _ in range(3, 6):
            new_chains = []
            for chain in chain_list:
                ...
                for s, e in rel_list:
                    if last == s:
                        new_chains.append(chain + [e])
            chain_list.extend(new_chains)
        return [list(c) for c in set(tuple(c) for c in chain_list)]
    tool_chains = build_tool_chains(relationships)
    print(f"Tool chains from updated KG: {len(tool_chains)}")
    return [{"chain_id": i+1, "chain": c} for i, c in enumerate(tool_chains)], tool_chains


def build_chain_retriever(tool_chains):
    chain_texts = [" -> ".join(c) for c in tool_chains]
    emb = OpenAIEmbeddings(model="text-embedding-3-small", openai_api_key=api_key, openai_api_base=URL)
    vs = FAISS.from_texts(chain_texts, emb)
    print(f"Retriever built: {len(chain_texts)} chains.")
    return vs, chain_texts
...
async def main():
    updated_kg = auto_update_knowledge_graph(output_file='KGoLNR_updated_L4.json')
    print("="*60)
    print("Step 1: Generating chains from updated KG...")
    _, raw_chains = load_and_generate_tool_graph(updated_kg)
    vs, chain_texts = build_chain_retriever(raw_chains)

    print("="*60)
    print("Step 2: Connecting MCP servers (L4: 18 servers, 141 tools)...")
    client = MultiServerMCPClient({
        ...
        "added_gis_data_L4": {"command": "python", "transport": "stdio", "args": ["added_gis_data_L4.py"]},
        "added_gis_geopandas_L4": {"command": "python", "transport": "stdio", "args": ["added_gis_geopandas_L4.py"]},
        "added_gis_pyproj_L4": {"command": "python", "transport": "stdio", "args": ["added_gis_pyproj_L4.py"]},
        "added_gis_pysal_L4": {"command": "python", "transport": "stdio", "args": ["added_gis_pysal_L4.py"]},
        "added_gis_rasterio_L4": {"command": "python", "transport": "stdio", "args": ["added_gis_rasterio_L4.py"]},
        "added_gis_shapely_L4": {"command": "python", "transport": "stdio", "args": ["added_gis_shapely_L4.py"]},
        "added_gis_utilities_L4": {"command": "python", "transport": "stdio", "args": ["added_gis_utilities_L4.py"]},
        "added_gis_visualize_L4": {"command": "python", "transport": "stdio", "args": ["added_gis_visualize_L4.py"]},
        "added_math_L4": {"command": "python", "transport": "stdio", "args": ["added_math_L4.py"]},
        "added_networkx_L4": {"command": "python", "transport": "stdio", "args": ["added_networkx_L4.py"]},
        "added_powerfactory_L4": {"command": "python", "transport": "stdio", "args": ["added_powerfactory_L4.py"]},
    })
    ...
    print("="*60)
    print("Step 3: Running tasks with auto-updated KG chains...")
    df = pd.read_excel("mcp tool chains_test.xlsx")
    descriptions = df["Task"].tolist()
    output_path = "MCP_agent_response_L4_141tools_autoupdate.xlsx"
    ...
    for i, desc in enumerate(descriptions):
        ...
        try:
            resp = await asyncio.wait_for(agent.ainvoke({"messages": [HumanMessage(content=combined)]}, config={"recursion_limit": 25}), timeout=TIMEOUT)
