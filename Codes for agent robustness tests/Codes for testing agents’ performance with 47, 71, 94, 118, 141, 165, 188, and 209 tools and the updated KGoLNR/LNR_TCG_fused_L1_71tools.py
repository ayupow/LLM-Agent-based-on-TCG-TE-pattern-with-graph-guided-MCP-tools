...
# ==========================================
# TCG_tool: Dynamic Tool Chain Generation from Knowledge Graph
# ==========================================
def load_and_generate_tool_graph(input_json_file='KGoLNR.json'):
    """
    Extract tool-node relationships from the knowledge graph JSON,
    build tool chains starting from 'convert_SHPfile_to_JSONfile',
    deduplicate and return both numbered chains and raw chain lists.
    """
...
def build_chain_retriever(tool_chains):
    """
    Build a FAISS vectorstore over tool chain texts for similarity-based retrieval.
    Returns (vectorstore, chain_texts).
    """
...
def retrieve_best_chain(task_description, vectorstore, chain_texts, k=1):
    """
    Given a task description, retrieve the k-best matching tool chain texts.
    Returns the best chain as a " -> " joined string.
    """
...
async def main():
    # --- Step 1: Generate tool chains dynamically from KGoLNR.json ---
    print("=" * 60)
    ...
    # --- Step 2: Connect to MCP servers (L1 config: 10 servers, 71 tools) ---
    print("=" * 60)
    ...
    # --- Step 3: Read tasks and dynamically retrieve tool chains from KG ---
    print("=" * 60)
    ...
    output_path = "MCP_agent_response_L1_71tools_fused.xlsx"
    ...
    for i, desc in enumerate(descriptions):
        ...
        # Dynamically retrieve the best matching tool chain from KG
